...
VehicleMotor.mrNew = function (vehicle, superFunc, minRpm, maxRpm, maxForwardSpeed, maxBackwardSpeed, torqueCurve, brakeForce, forwardGears, backwardGears, minForwardGearRatio, maxForwardGearRatio, minBackwardGearRatio, maxBackwardGearRatio, ptoMotorRpmRatio, minSpeed)
    ...
    newMotor.mrTransmissionNeedShiftUpMaxTime = 400
    ...
    if not waitingForTimer then
        ...
        if self.backwardGears or self.forwardGears then
            ...
            if self:getUseAutomaticGearShifting() and self:getManualClutchPedal() <= 0.5 then
                ...
                if self.autoGearChangeTimer == 0  and (self.mrPreventAutoGearShiftTimer<=0 or self.mrPreventAutoGroupShiftTimer<=0 or justChangedDirection) then
                    ...
                    if math.abs(self.differentialRotSpeed) < 0.03 or Vehicle.mrGetIdleTurningActive(self.vehicle) or justChangedDirection then --0.03 = 1.08kph
                        ...
                        if acceleratorPedal==0 or justChangedDirection or self.mrBestStartGearSelected==0 then
                            ...
                            if not force and newGear==self.previousGear then
    ...
    if self.gearShiftMode == VehicleMotor.SHIFT_MODE_MANUAL_CLUTCH then
        if self.backwardGears or self.forwardGears then
            ...
            if self.currentGears[self.gear] ~= nil then
                ...
                local differentialRotSpeed = math.max(math.abs(self.differentialRotSpeed), 0.01)
...
VehicleMotor.mrFindBestGearCombination = function(self, curGear1, gearbox1, gearbox1active, curGear2, gearbox2, gearbox2active, acceleratorPedal, dt)
    ...
    local newGear1 = curGear1
    local newGear2 = curGear2
    ...
    local curGear1Ratio = gearbox1[curGear1].ratio
    local curGear2Ratio = 1
    if gearbox2active then
        curGear2Ratio = gearbox2[curGear2].ratio
    end

    local curGear1Sign = math.sign(curGear1Ratio)
    local curGear2Sign = math.sign(curGear2Ratio)

    curGear1Ratio = math.abs(curGear1Ratio)
    curGear2Ratio = math.abs(curGear2Ratio)

    local curGlobalRatio = curGear1Ratio * curGear2Ratio
    ...
    elseif self.lastMotorExternalTorque>0 then
        ...
        maxRpmWanted = math.min(maxRpmWanted, maxRpmNotGoverned)
        if engineRpm<minRpmWanted then
    ...
    if not gearFound and (forceLug or engineRpm<minRpmWanted) then --and self.mrTransmissionLastShiftDirection<=0 then
        ...
        elseif self.motorRotAcceleration<10 or engineRpm/minRpmWanted<0.75 then --less than 100rpm/s
            self.mrTransmissionLugTime = self.mrTransmissionLugTime + dt
        end

        if self.mrTransmissionLugTime>self.mrTransmissionLugMaxTime then
    ...
    if not gearFound and self.vehicle.mrPreventAutoShiftTimer~=nil and self.vehicle.mrPreventAutoShiftTimer>0 then
        gearFound = true
    end
    ...
    if not gearFound and absAccPedal>0.7 and engineRpm>0.5*(minRpmWanted+maxRpmWanted)*(0.5+0.5*absAccPedal)  then --only try changing gear up if acc above 70%
        ...
        local toolSpdLimit = self:getSpeedLimit() --kph
        local minTotalGearRatioPossible = 0  --we don't want to shift up if we end with a gear to "quick" to keep the speedlimit and the min engine rpm wanted
        if toolSpdLimit>0 and toolSpdLimit<99 then
            minTotalGearRatioPossible = 0.37699*minRpmWanted/toolSpdLimit -- formula = 3.6 * rpm * pi / (30 * kph)
        end
